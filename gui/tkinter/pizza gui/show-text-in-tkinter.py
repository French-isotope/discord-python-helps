...
for category in database:
    ...
    for id, element in enumerate(database[category]["elements"]):
        ...
        print(database[category]["elements"][element]["quantity"].get())
        Entry(database[category]["position"]["f"],
              font=(font, size, grease),
              width=3,
              textvariable=database[category]["elements"][element]["quantity"]).grid(row=id, column=1)
