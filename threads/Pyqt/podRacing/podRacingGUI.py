...
#io.StringIO.reconfigure(encoding="utf-8")
    ...
    def fetch_RSS(self):
        ...
        if rss_url == '':
            ...
            try: ## GOOD!
                ...
                rss_data = BeautifulSoup(rss_feed.content.encode('utf-8-sig'), features="lxml")
...
#                    episodesText.write(rss_item['description']) ## DESCRIPTION TEXT
            ...
            with open(self.input_file, "w") as inputText:
                #print(f'rss_data \n\n\n\n {type(rss_data)}\n\n\n\n ')
                #print(f'rss_data \n\n\n\n {BeautifulSoup(str(rss_data), "xml")}\n\n\n\n ')
                # bs_content = bs(content, "lxml")
                try:
                    inputText.write(BeautifulSoup(str(rss_data), "xml").prettify(formatter="html").replace('\ufeff', ''))
                except Exception as e:
                    print(f'get audio links (line 270) : {e}')
                else:
                    print("(line 270) :ok")
...
#            rss_html = rss_html.to_string().encode('utf-8')
#            print(rss_html)

#            rss_html["title"] = rss_html["title"].str.replace('\ufeff', '')
            rss_html["description"] = str(rss_html["description"].str.replace('\ufeff', ''))
            rss_html["description"] = str(rss_html["description"].str.replace('\u200b', ''))
#            rss_html["description"] = rss_html["description"]
#            rss_html["description"].str.replace('CDATA', 'LOL')
#            print(rss_html["description"].str.replace('CDATA', 'LOL'))
...
#            print(f'rss_data \n\n\n\n {rss_html}\n\n\n\n ')
            ...
            try:
                ...
                print(f'(line {line_debugged}) : {e}')
            else:
                print(f'(line {line_debugged}) : ok')
