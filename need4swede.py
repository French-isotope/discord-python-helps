...
from PyQt6.QtWidgets import QApplication, QStatusBar, QWidget, QLabel, QLineEdit, QPushButton, QProgressBar, \
    QMessageBox, QFileDialog, QVBoxLayout, QHBoxLayout, QCheckBox
...
class PodRacingGUI(QWidget):
    ...
    def threadRunner(self):
        ...
        self.worker.signals.finished.connect(
            self.thread_finished)  # connect finish signal of our thread to thread_complete
...
'''
        ...
        self.merge_html(f"{directory}/{title}.html", f"{directory}/assets/style.css", f"{directory}/assets/script.js",
                        directory, title)
        return
